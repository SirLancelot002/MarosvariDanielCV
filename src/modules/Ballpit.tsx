...
class X {
  ...
  #initRenderer() {
    ...
    this.canvas.width = Math.max(1, Math.round(size.width * (window.devicePixelRatio || 1)));
    this.canvas.height = Math.max(1, Math.round(size.height * (window.devicePixelRatio || 1)));
    ...
    try {
      ...
      console.warn('Ballpit: WebGL renderer could not be initialized. Falling back to a non-3D mode.', error);
  ...
  #getInitialSize() {
    ...
    if (this.#config.size === 'parent' && this.canvas.parentNode) {
      ...
      return { width: parent.offsetWidth || window.innerWidth, height: parent.offsetHeight || window.innerHeight };
    }
  ...
  #onIntersection(entries: IntersectionObserverEntry[]) {
    ...
    entries[0].isIntersecting ? this.#startAnimation() : this.#stopAnimation();
  }
...
const XConfig = {
  count: 200,
  colors: [0x7c3aed, 0x06b6d4, 0xff4fd8],
...
function onTouchStart(e: TouchEvent) {
  if (e.touches.length > 0) {
    ...
    if (overAnyElement) e.preventDefault();
  }
}
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer | undefined, params: Partial<typeof XConfig> = {}) {
    const config = { ...XConfig, ...params };
    let materialParams: Record<string, unknown> = { ...config.materialParams };

    if (renderer) {
      const roomEnv = new RoomEnvironment();
      const pmrem = new PMREMGenerator(renderer);
      const envTexture = pmrem.fromScene(roomEnv).texture;
      materialParams = { envMap: envTexture, ...materialParams };
    }
